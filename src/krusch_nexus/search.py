"""
KruschNexus Hybrid Search Engine
================================
...
Zero cloud dependencies. Strictly enforces workspace isolation.
"""
...
def format_citation(filename: str, page_number: int, header: Optional[str] = None) -> str:
    """Format canonical citation string, e.g. '{filename} p.{n} § {header}'."""
    cit = Citation(filename=filename, page_number=page_number, header=header)
...
def hybrid_search(
    ...
    """
    if not allow_cross_workspace and (not workspace_id or workspace_id <= 0):
        raise WorkspaceRequiredError(
            "Search requires an explicit workspace ID. Refusing global search across workspaces."
        )
    ...
    # 6. Rank by final score and build canonical ChunkHit models
